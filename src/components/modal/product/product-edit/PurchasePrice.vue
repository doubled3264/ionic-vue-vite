...
async function editProduct() {
   ...
   const productData = {
      id: props.productId,
      item: { admin: admin.id, purchase_price: product.value.purchase_price },
   }
   /* await store */
...
watch(
   ...
   () => {
      ...
      product.value = store.getters['product/getPurchasePrice']
      setTitle()
   }
)
</script>
               ...
               <ion-row>
                  <ion-col>
                     <custom-input
                        label="harga modal"
                        input-mode="numeric"
                        v-model:input-value="product.purchase_price"
